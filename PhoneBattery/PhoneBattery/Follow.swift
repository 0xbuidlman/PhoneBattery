...
class Follow: NSObject {
    ...
    func actionSheet(accounts: [ACAccount]?, username: String) -> UIAlertController? {
        if let accountArray = accounts {
            let actionSheet = UIAlertController(title: nil, message: String(format: NSLocalizedString("CHOOSE_ACCOUNT", comment: ""), username), preferredStyle: .actionSheet)
                ...
                actionSheet.addAction(UIAlertAction(title: account.username, style: .default, handler: { (action) in
                    self.follow(username: username, account: account, completionHandler: { (success, error) in
                        actionSheet.dismiss(animated: true, completion: {
                            if error != nil {
                                let alertController = UIAlertController(title: NSLocalizedString("ERROR", comment: ""), message: String(format: NSLocalizedString("FOLLOW_ERROR", comment: ""), username, error!.localizedDescription), preferredStyle: .alert)
